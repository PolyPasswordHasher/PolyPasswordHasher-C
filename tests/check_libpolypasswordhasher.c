...
START_TEST(test_pph_unlock_password_data_full_range) {
  ...
  for( i = 0; i < MAX_USERNAME_LENGTH-1; i++){
    ...
    usernames[i] = malloc(sizeof(*usernames[i])*(MAX_USERNAME_LENGTH+1));
    passwords[i] = malloc(sizeof(*passwords[i])*(MAX_PASSWORD_LENGTH+1));
    ...
    // the test failed unless I changed it to strlen(passwords[i])
    // the reason is that function RAND_bytes could result in having one
    // (or more) of the generated bytes inside the password equals to zero (which is NULL)
