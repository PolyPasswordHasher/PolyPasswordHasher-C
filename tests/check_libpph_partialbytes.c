...
START_TEST(test_pph_destroy_context_AES_key)
{
  ...
  uint8 partial_bytes = 2;// this function is part of the non-partial bytes
                          // suite
...
Suite * polypasshash_partial_bytes_suite(void)
{
  ...
  tcase_add_test (tc_partial,test_pph_init_context_AES_key);
  tcase_add_test (tc_partial,test_pph_destroy_context_AES_key);
